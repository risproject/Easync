    ...
    return (
        <nav className="bg-white shadow-sm border-b border-slate-200">
            <div className="w-full px-4 py-3 flex items-center justify-between gap-3">
                <button type="button" onClick={onPowerToggle}
                ...
                <div className="relative" ref={menuRef}>
                    <button type="button" onClick={() => setOpen((v) => !v)} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 transition-colors" >
                        <span className="max-w-[140px] truncate">{displayName}</span>
                        <svg viewBox="0 0 20 20" className={`w-4 h-4 text-slate-500 transition-transform ${open ? "rotate-180" : ""}`} fill="currentColor" aria-hidden="true">
